...
TEST_F(ProjectsBaseline, copy_if_else)
{
  ...
  cudf::size_type const total_rows = 400 * 1'000'000;

  cudf::size_type const n_cols = num_streams_arg;
  cudf::size_type const n_rows = total_rows / n_cols;

  auto stream = cudf::get_default_stream();
  auto mr     = cudf::get_current_device_resource_ref();

  // create multiple streams
  auto streams = cudf::detail::fork_streams(stream, num_streams_arg);
  ...
  printf("allocate %d big columns time: \n", n_cols);
  ...
  printf("ProjectsBaseline: Processed %d rows and %d columns using %d streams\n",
         n_rows,
         n_cols,
         num_streams_arg);
}
...
int main(int argc, char** argv)
{
  ...
  if (argc > 1) { num_streams_arg = std::atoi(argv[1]); }
