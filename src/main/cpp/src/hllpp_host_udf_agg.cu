...
#include "hllpp_aggregation_utils.hpp"
...
struct hllpp_udf : cudf::host_udf_base {
  ...
  [[nodiscard]] bool is_equal(host_udf_base const& other) const override
  {
    ...
    return dynamic_cast<hllpp_udf const*>(&other) != nullptr;
  }

  [[nodiscard]] std::size_t do_hash() const override
  {
    return std::hash<std::string>{}({"hllpp_udf"});
  }

  [[nodiscard]] std::unique_ptr<host_udf_base> clone() const override
  {
    return std::make_unique<hllpp_udf>();
  }
...
std::unique_ptr<cudf::host_udf_base> create_hllpp_reduction_host_udf()
{
  return std::make_unique<hllpp_udf<cudf::reduce_aggregation>>();
}

std::unique_ptr<cudf::host_udf_base> create_hllpp_segmented_reduction_host_udf()
{
  return std::make_unique<hllpp_udf<cudf::segmented_reduce_aggregation>>();
}

std::unique_ptr<cudf::host_udf_base> create_hllpp_groupby_host_udf()
{
